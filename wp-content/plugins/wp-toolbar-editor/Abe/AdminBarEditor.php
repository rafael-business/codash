...
class Abe_AdminBarEditor {
		...
		foreach($this->defaultNodes as $wpNode) {
			if ( isset($this->mergedNodes[$wpNode->id]) ) {
				...
				if ($previousNodeId === null) {
					...
					$this->mergedNodes = array($node->id => $node) + $this->mergedNodes;
				} else {
	...
	protected function saveCustomMenu($nodes) {
		...
		$oldCustomMenu = $isWpmlActive ? $this->loadCustomMenu() : array();
	...
	protected function sortNodesDepthFirst($nodes) {
		...
		$children = array($tempRootId => array());
	...
	public function displayEditorPage() {
		...
		$loginsToInclude = array($currentUser->user_login);
	 ...
	 *
	 * @param Puc_v4p8_Update $pluginInfo
